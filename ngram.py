...
if choice == '1':
    ...
    print "Choose one or two words/phrases and a category for each, and a name under which the plot should be saved."
    ...
    print "Please allow the program to run for around 10 minutes for one word/phrase, around 15 minutes for two."
word = raw_input("Enter first word/phrase")
...
word2 = raw_input("Enter second word/phrase (0 if none)")
...
for d in data:
    if category == 'Все':
        if d[3].find(word) != -1:
            x.append(datetime.strptime(d[2], "%Y-%m-%d %H:%M"))
...
if (word2 != '0') and (category2 != '0'):  
    for d in data:
        if category2 == 'Все':
            if d[3].find(word2) != -1:
                x2.append(datetime.strptime(d[2], "%Y-%m-%d %H:%M"))
